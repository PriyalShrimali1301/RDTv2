...
//        printf("Sent to B %s seq num %d\n",buffer[packet.seqnum].packet.payload, packet.seqnum);
...
//        printf("Buffer for pos %d window size from ( %d to %d ) \n", packet.seqnum, send_base, send_base+window_size-1);
        buffer[packet.seqnum].packet = packet;
...
//                    printf("Sent counter : %d send_base %d\n", q, send_base);
...
//highgate {/local/Spring_2023/atishkum/cse489589_assignment2/RDT/atishkum} > ../grader/run_experiments -p ../atishkum/sr -o sr_2ex3_2.csv -m 1000 -t 50 -c 0.2 -l 0.2 -w 100